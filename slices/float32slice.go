...
func (c Float32Slice) MapAsync(cb func(int, float32, chan []interface{})) Float32Slice {
...
func (c Float32Slice) MapAsyncInterface(cb func(int, float32, chan []interface{})) InterfaceSlice {
	mapChan := make(chan []interface{}, len(c))
	for k, v := range c {
		go cb(k, v, mapChan)
	}
